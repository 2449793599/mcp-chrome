...
export class Server {
    ...
    private async setupPlugins(): Promise<void> {
        ...
        await this.fastify.register(cors, {
            origin: (origin, cb) => {
                ...
                if (!origin) { // Allow requests with no origin (e.g., curl, server-to-server)
                    return cb(null, true);
                }

                const allowed = SERVER_CONFIG.CORS_ORIGIN.some((pattern) =>// Check if origin matches any pattern in whitelist
    ...
    private setupExtensionRoutes(): void {
        ...
        this.fastify.get('/ask-extension', async (request: FastifyRequest<{ Body: ExtensionRequestPayload }>, reply: FastifyReply) => {

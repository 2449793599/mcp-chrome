...
REM 这是一个启动包装脚本，用于在WINDOWS环境下可靠地找到并启动NODE.JS服务
...
REM 错误日志：记录NODE.JS的STDERR输出
set "STDERR_LOG=%LOG_DIR%\native_host_stderr_windows_%TIMESTAMP%.log"

REM 环境信息记录：记录所有可能影响NODE.JS查找的环境变量和路径
...
REM NODE.JS发现逻辑（核心部分）
set "NODE_EXEC="
set "NODE_EXEC_SOURCE="

REM Priority 0: 允许用户通过环境变量强制指定NODE.JS路径(CHROME_MCP_NODE_PATH)
...
REM Priority 1.5: 相对路径回退（查找脚本上层三级的NODE.EXE）
if not defined NODE_EXEC (
